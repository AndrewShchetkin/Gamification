...
public class HexMapEditor : MonoBehaviour
{
	...
	// Удалить
	//static bool hexGridReady;
	//public static bool HexGridReady
	//   {
	//	set
	//       {
	//		hexGridReady = value;
	...
	{        
		...
		if (Input.GetMouseButtonDown((int)PointerEventData.InputButton.Left) && !EventSystem.current.IsPointerOverGameObject())
		{
            ...
            else
            {
				...
				if (neighbors.Any(n => n?.OwnerId == GameController.PlayerTeam.id) && string.IsNullOrEmpty(cell.OwnerId))
                {
					Capture(cell);
				}			
			}			
    ...
    public void CaptureCell(HexCell cell)
	{
		if (cell)
		{
			var playerTeam = GameController.PlayerTeam;
	...
	{
		Debug.Log("Начата загрузка карты...");
#if UNITY_WEBGL == true && UNITY_EDITOR == false
		LoadMap();
#endif
	}
